...
public class ThymeleafController2 {
    ...
    public String variable(Model model) {
        ...
        List<User> list = new ArrayList<>(); // 리스트에 담아서 전달
        list.add(userA);
        list.add(userB);
        ...
        model.addAttribute("user", userA); // 객체 1개만 전달
        model.addAttribute("users", list); // ArrayList 전달
        model.addAttribute("userMap", map); // hashMap 전달
