...
@WebServlet("/basket3_1")
public class BasketServlet3_1 extends HttpServlet {@SuppressWarnings("unchecked")
@Override
	...
	for(int i=0; i < count.length; i++) {
		if(count[i] != 0) {
			obj.put("p"+(i+1), count[i]);	
		}			
	}	
	session.setAttribute("cnt", obj);
	
	out.println("<h3>선택한 상품 리스트</h3>");				
	out.println("===================");
	out.print("<ul>");
	for(int i=0; i < count.length; i++) {
		if(count[i] != 0) {
