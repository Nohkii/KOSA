...
public class StringTest {
	public static void main(String[] args) {
		...
		System.out.println(str2.lastIndexOf("f"));      //3
		...
		String[] ary = str3.split(" ");   // [java,jdbc,html5,css,javascript,servlet,jsp,ajax]
		
		for(int i=0; i < ary.length; i++){
			System.out.println(ary[i]); // java jdbc html5 css javascript servlet jsp ajax
		}		
		char ch[] = str3.toCharArray(); 
		System.out.println(str3.length() + " ---- " + ch.length); // 46 -----39
		for(int i=0; i < ch.length; i++){
			System.out.print(ch[i] + " "); // j a v a j d b c h t m l 5 c s s j a v a s c r i p t s e r v l e t j s p a j a x
		}
	}
}
